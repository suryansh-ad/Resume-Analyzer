...
export async function runCrawler() {
  ...
  try {
    ...
    // 4. Cleanup expired opportunities for successfully crawled companies
    logMessage("Cleaning up expired opportunities...");
    let expiredJobsCount = 0;
    let expiredInternshipsCount = 0;

    for (const companyName of successfullyCrawledCompanies) {
      ...
      // Find all existing jobs for this company in DB
      ...
      for (const job of existingJobs) {
        if (!activeUrls.has(job.applyUrl)) {
          await prisma.job.delete({ where: { id: job.id } });
          expiredJobsCount++;
        }
      }

      // Find all existing internships for this company in DB
      const existingInternships = await prisma.internship.findMany({
        where: { companyId: dbCompany.id },
        select: { id: true, applyUrl: true }
      });

      for (const intern of existingInternships) {
        if (!activeUrls.has(intern.applyUrl)) {
          await prisma.internship.delete({ where: { id: intern.id } });
          expiredInternshipsCount++;
        }
      }
    }
