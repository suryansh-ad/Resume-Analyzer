      ...
      <div className="grid gap-8 rounded-[2rem] border border-white/10 bg-slate-900/70 p-6 shadow-glass backdrop-blur-xl md:grid-cols-[0.9fr_1.1fr] md:p-8">
        ...
        <form onSubmit={handleSubmit} className="rounded-[1.5rem] border border-white/10 bg-slate-950/70 p-5">
          ...
          <div className="mt-5 flex flex-wrap items-center justify-center gap-3 text-sm">
            {!isReset ? (
              <button type="button" onClick={() => changeMode("reset")} className="text-cyan-200 transition hover:text-white">
                Forgot password?
              </button>
            ) : (
              <button type="button" onClick={() => changeMode("signin")} className="text-cyan-200 transition hover:text-white">
                Back to sign in
              </button>
            )}
          </div>
